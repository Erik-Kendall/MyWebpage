...
import AvailabilityCalendar from "../components/AvailabilityCalendar"; // Make sure this import is correct
...
const Schedule = () => {
    ...
    const [activeTab, setActiveTab] = useState('my-availability'); // Changed default tab to 'my-availability'
    ...
    useEffect(() => {
        // Fetch events whenever token or user changes, or if activeTab becomes 'my-events' or 'my-availability'
        if (token && user) {
            if (activeTab === 'my-events' || activeTab === 'my-availability' || activeTab === 'event-details') {
                fetchUserEvents();
            }
        } else {
            // If user is not logged in, clear events and show message
            setUserEvents([]);
                ...
                <Card.Body>
                    ...
                    >
                        ...
                        <Tab eventKey="my-availability" title="My Availability">
                            <div className="mt-3">
                                {/* Pass the eventsByDate map to your calendar component */}
                                <AvailabilityCalendar
                                    eventsByDate={eventsByDate}
                                    onEventClick={handleEventClick} // Pass the handler to open event details
                                />
                            </div>
                        </Tab>
                        ...
                        <Tab eventKey="event-details" title="Event Details">
                            <EventDetailsContent
                                ...
                                // Ensure EventDetailsContent can navigate back or close
                                // You might need to add a prop to tell it to switch tabs
                                onGoBack={() => setActiveTab('my-events')} // Add this prop for a "Go Back" button if needed
                            />
                        </Tab>
                    </Tabs>
                </Card.Body>
