...
const ProfilePage = () => {
    // Destructure `loading` (renamed to authLoading) and `isAuthenticated` from useAuth
    const { user, token, updateUserData, loading: authLoading, isAuthenticated } = useAuth(); // <--- MODIFIED: Added authLoading and isAuthenticated
    ...
    const [loading, setLoading] = useState(true); // Overall page loading state (distinct from authLoading)
    ...
    useEffect(() => {
        const loadAllData = async () => {
            setLoading(true); // Start overall page loading (for this component's data fetches)
        ...
        // MODIFIED: Conditional fetching based on authLoading and isAuthenticated
        if (!authLoading) { // Wait until AuthContext has finished its initial loading check
            if (isAuthenticated && user) { // Proceed only if authenticated and user object exists
                loadAllData();
            } else {
                setLoading(false); // If authLoading is done but not authenticated, stop ProfilePage loading
                ...
                console.log('loadAllData: Auth finished, not authenticated. Cleared data and set loading to false.');
            }
        }
        ...
        // MODIFIED DEPENDENCIES: Added authLoading, isAuthenticated to ensure re-evaluation when auth state changes
    }, [token, authLoading, isAuthenticated, user, fetchProfileData, fetchFriendsData, fetchUserGames, profilePicPreview]);
    ...
    // MODIFIED: Combine authLoading with component's own loading
    ...
    // This condition should now be more reliably hit after authLoading is false
    if (!user) {

        console.log("ProfilePage: user object before rendering ProfileInfoTab:", user); // ADD THIS LINE
                    ...
                    >
                        ...
                        <Tab eventKey="profile" title="My Profile">
                            <ProfileInfoTab
                                ...
                                error={error} // Pass ProfilePage's error down to ProfileInfoTab
                                ...
                                setError={setError} // <--- Pass setError down for ProfileInfoTab to use
                                ...
                                navigate={navigate} // <--- PASS THE NAVIGATE FUNCTION HERE
                            />
                        </Tab>
                        ...
                        <Tab eventKey="accountSettings" title="Account Settings">
                            <AccountSettingsTab
                                ...
                                setLoading={setLoading} // Note: This `setLoading` is ProfilePage's loading state. AccountSettingsTab might need its own loading state.
                            />
                        </Tab>
