...
// Removed '../App.css' import from here. It's better for App.js to import App.css once.
// We will manage calendar specific styles in Schedule.css as planned.
                    ...
                    {eventsForThisDate && eventsForThisDate.length > 0 && (
                        <div className="events-marker" onClick={(e) => {
                            ...
                            if (onEventClick && eventsForThisDate[0]) {
                                // FIX: Use event.event_id instead of event.id
                                onEventClick(eventsForThisDate[0].event_id);
                            }
                        }}>
                            {/* Changed Badge background to 'warning' for better visibility, adjust as needed */}
                            <Badge pill bg="warning">{eventsForThisDate.length}</Badge>
                        </div>
                    )}
                </div>
            );
    ...
    return (
        <div className="availability-calendar">
            ...
            <div className="legend mt-3">
                ...
                <span className="legend-item events-legend"><Badge pill bg="warning">N</Badge></span> Events
            </div>
        </div>
    );
}
