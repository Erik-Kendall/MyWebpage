...
const SearchPage = () => {
    ...
    const handleUserSearch = useCallback(async (e) => {
        ...
        try {
            const response = await fetch(`http://localhost:3001/api/users/search?term=${encodeURIComponent(userSearchTerm.trim())}`, {
                method: 'GET',
                headers: {
    ...
    const handleGameSearch = useCallback(async (e) => {
        ...
        try {
            const response = await fetch(`http://localhost:3001/api/games/search?q=${encodeURIComponent(gameSearchQuery.trim())}`);
            if (!response.ok) {
                const errorData = await response.json();
