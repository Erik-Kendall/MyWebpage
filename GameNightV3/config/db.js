...
const DB_PATH = path.join(__dirname, '..', 'gamenight.db'); // This correctly points to 'server/gamenight.db'

async function initializeDatabase() {
    let db;
    try {
        // First, connect to the database. If it doesn't exist, it will be created.
        db = await open({
            filename: DB_PATH,
        ...
        // Users Table - ADDING EMAIL COLUMN
        await db.run(`
            CREATE TABLE IF NOT EXISTS users (
                id TEXT PRIMARY KEY,
                username TEXT UNIQUE NOT NULL,
                email TEXT UNIQUE, -- ADDED THIS LINE
        ...
        const userColumnsToAdd = [
            'email TEXT UNIQUE', // ADDED THIS LINE FOR MIGRATION
            'isAdmin INTEGER DEFAULT 0',
            'firstName TEXT',
            'lastName TEXT',
            'favoriteGames TEXT',
        ...
        process.exit(1); // Exit if DB connection or schema setup fails
    }
}
